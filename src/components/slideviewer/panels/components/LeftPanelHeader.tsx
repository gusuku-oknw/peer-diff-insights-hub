...
const LeftPanelHeader: React.FC<LeftPanelHeaderProps> = ({
  ...
  return (
    <div className={`${isVeryNarrow ? 'p-2' : 'p-4'} border-b border-gray-200 flex-shrink-0 bg-gradient-to-r from-blue-50 to-indigo-50`}>
      <div className="flex items-center justify-between mb-2">
        <h2 className={`${isVeryNarrow ? 'text-sm' : 'text-lg'} font-semibold text-gray-800 flex items-center min-w-0`}>
          <History className={`${isVeryNarrow ? 'h-4 w-4 mr-1' : 'h-5 w-5 mr-2'} text-blue-600 flex-shrink-0`} />
      ...
      <div className="flex items-center justify-between">
        <p className={`${isVeryNarrow ? 'text-xs' : 'text-sm'} text-gray-600 flex items-center truncate`}>
          <GitBranch className={`${isVeryNarrow ? 'h-3 w-3 mr-1' : 'h-4 w-4 mr-2'} text-blue-500 flex-shrink-0`} />
          <span className="truncate">{isVeryNarrow ? currentBranch : `現在のブランチ: ${currentBranch}`}</span>
        </p>
      </div>
