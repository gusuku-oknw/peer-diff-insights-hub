    ...
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
            ...
            {slide.thumbnail ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                <div className="text-center">
                  <svg 
                    className={`${isSmall ? 'w-4 h-4' : isMedium ? 'w-6 h-6' : 'w-8 h-8'} text-gray-300 mx-auto mb-1`} 
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
          ...
          <div className="text-center">
            {/* Slide number badge - responsive sizing */}
            ...
            {/* Status indicators - only show on medium+ sizes */}
            {!isSmall && (
              <div className="flex items-center justify-center gap-2 mt-2">
                ...
                {showStudentFeatures && commentCount > 0 && (
                  <div className="flex items-center text-purple-500">
                    <MessageSquare className={`${isMedium ? 'h-3 w-3' : 'h-3 w-3'} mr-1`} />
                    <span className={`font-medium ${isMedium ? 'text-xs' : 'text-xs'}`}>{commentCount}</span>
                  </div>
                )}
                ...
                {!showStudentFeatures && slide.hasComments && (
                  <div className="flex items-center text-purple-500">
                    <MessageSquare className={`${isMedium ? 'h-3 w-3' : 'h-3 w-3'}`} />
                  </div>
                )}
              </div>
            )}
